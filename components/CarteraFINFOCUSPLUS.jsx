...
async function fetchSeriesEvolucion() {
  ...
  // 2) Buscar índices de columnas para FINFOCUS+
  const idxMes = findCol(headers, 'Mes');
  const idxColchonStart = findCol(headers, 'FINFOCUS+ (colchón)');
  const idxTeoricaStart = findCol(headers, 'FINFOCUS+ (teórica)');
  const idxRealStart = findCol(headers, 'FINFOCUS+ (real)');
  ...
  if (
    ...
    console.warn(
      'No se encontraron todas las columnas de FINFOCUS+ en teorica_versus_real'
    );
    return { labels: [], colchon: [], teorica: [], real: [] };
  }
...
// ======================
// Fetch carteras desde FinancialData (FINFOCUS+)
// ======================
async function fetchSheetTickers() {
  ...
  const idxFinfocusPesos = findCol(headers, 'FINFOCUS+');

  if (idxTicker === -1 || idxFinfocusPesos === -1) {
    console.warn('No se encontraron columnas Ticker / FINFOCUS+ en "carteras"');
    return [];
  }
...
async function fetchPreciosARS() {
  const [rc, rb, rn, rced] = await Promise.allSettled([
    ...
    fetch('https://data912.com/live/arg_cedears', { mode: 'cors' }),
  ]);
  ...
  const cedears = await asJson(rced).catch(() => []);

  const map = {};

  const addList = (arr, isCedear) => {
    for (const it of arr) {
      const sym = normSym(
      ...
      if (sym && isFinite(px)) {
        map[sym] = { price: px, isCedear };
      }
    }
  };

  // Bonos / ONs / notas → isCedear = false
  addList(corp, false);
  addList(bonds, false);
  addList(notes, false);

  // CEDEARs → isCedear = true
  addList(cedears, true);
...
useEffect(() => {
  ...
  async function loadComposicion() {
    try {
      ...
      // 1) Cantidades por ticker desde FinancialData (FINFOCUS+)
      const items = await fetchSheetTickers();
      if (!isMounted) return;

      if (!items.length) {
        setCompError(
          'No se encontraron tickers con cantidad > 0 en la serie "carteras" (FINFOCUS+).'
        );
        setRowsComp([]);
        setTotalGeneral(0);
        return;
      ...
      // 2) Precios de mercado (ahora devuelven { price, isCedear })
      ...
      const rows = items.map(({ ticker, cantidad }) => {
        ...
        if (esLiquidez) {
          // Interpretamos FINFOCUS+ como monto de liquidez ya en USD
        ...
        // ==== PRECIOS DE MERCADO (bonos / CEDEARs, etc.) ====
        let precioInfo = preciosMap[symNorm];

        if (!precioInfo) {
          const key = Object.keys(preciosMap).find((k) =>
            k.startsWith(symNorm)
          );
          if (key) precioInfo = preciosMap[key];
        }

        const precioNum =
          precioInfo && isFinite(precioInfo.price)
            ? precioInfo.price
            : 0;

        const esCedearMercado = precioInfo ? !!precioInfo.isCedear : false;

        // Bonos/ONs: cotizan sobre 100 → dividir por 100
        // CEDEARs: precio por unidad → NO dividir por 100
        const totalInvertido = esCedearMercado
          ? cantidad * precioNum
          : cantidad * (precioNum / 100);
      ...
      const rowsConPct = rows.map((r) => ({
        ...r,
        porcentaje:
          totalGeneral > 0 ? (r.totalInvertido / totalGeneral) * 100 : 0,
      }));
      ...
      if (isMounted)
        setCompError(
          'Error al cargar composición desde FinancialData. Ver consola.'
        );
    } finally {
...
useEffect(() => {
  ...
  async function loadBoletos() {
    try {
      ...
      const idxMes = findCol(headers, 'Mes');
      const idxPesos = findCol(headers, 'FINFOCUS+');

      if (idxYear === -1 || idxMes === -1 || idxPesos === -1) {
        if (isMounted) {
          setBoletosError(
            'No se encontraron columnas Año / Mes / FINFOCUS+ en la serie "boletos".'
          );
          setBoletosData({});
        }
        return;
          ...
          <Card>
            <CardHeader className="text-center">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Compras acumuladas y reinversiones desde el 01-04-2025
              </p>
            </CardHeader>
            <CardContent>
              {compLoading && (
                      ...
                      <tbody>
                        {rowsComp.map((row, idx) => (
                          ...
                          >
                            ...
                            <td className="px-3 py-2 border border-slate-200">
                              {row.precio != null
                                ? `USD ${fmtMoney(row.precio)}`
                                : row.isLiquidez
                                ? ''
                                : 'N/D'}
                            </td>
                            <td className="px-3 py-2 border border-slate-200">
                              USD {fmtMoney(row.totalInvertido || 0)}
                            </td>
                            <td className="px-3 py-2 border border-slate-200">
